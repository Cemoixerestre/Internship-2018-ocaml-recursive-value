...
The next part of my work consisted in developping such a system.
...
\begin{itemize}
  ...
  \item As the expression $e_1$ is evaluated in the surounding mode $m$ (there
    is no delayed evaluation), it is evaluated at least in the mode
...
expression of the elements of the records. The element are typed under an
environment \lstinline|m'| with the function \lstinline|expression|, and their
environments are joined.

% TODO: give a reference about the optimization of float records.
% TODO: complete
As we can see, there are many more subtilities in the implementation of the
check than in the rules, which cover only a restricted subset of OCaml. For
example, because of an optimization performed by the compiler, there is a
special treatment to records of floats.
