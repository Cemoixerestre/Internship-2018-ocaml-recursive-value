...
% 
  ...
  \\
  \infer*{\Gamma \vdash e_1: A \\
          \Gamma \vdash e_2: B}
         {\Gamma \vdash \tapply{e_1}{e_2}: \inspect(A + B)}

  \infer*{\Gamma \vdash e: A}
         {\Gamma \vdash \tfun x e: \delay(A)}
  \\
  \infer*
    {(\Gamma \vdash e_i: A_i)^i}
    {\Gamma \vdash K(e_1, ..., e_n): \guard(A_1 + ... + A_n)}
  \\
  \infer*
    {\Gamma \vdash e_1: A \\
     \Gamma, x : A \vdash e_2: B}
    {\Gamma \vdash \tlet{x}{e_1}{e_2}: discard(A) + B}
\end{mathpar}
...
$\Gamma$. If a variable is used in mode $\Unguarded$ or $\Deref$ in one of
